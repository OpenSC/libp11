...
int PKCS11_keygen(PKCS11_TOKEN *token, PKCS11_KGEN_ATTRS *kg)
{
	...
	unsigned char bin_id[MAX_PIN_LENGTH] = {0};
	size_t key_id_len = kg->id_len;
	if (kg->key_id) {
		if(!hex_to_bin(kg->key_id, bin_id, &key_id_len))
			return -1;
	}
	...
	case EVP_PKEY_RSA:
		return pkcs11_rsa_keygen(slot, kg->kgen.rsa->bits,
				kg->key_label, bin_id, key_id_len, kg->key_params);
	case EVP_PKEY_EC:
